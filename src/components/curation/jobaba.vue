<template>
    <div v-if="jobDetail" class="container">
        <div class="top">
            <div>
                <h4>{{ jobDetail.ENTRPRS_NM }}</h4>
                <h2>{{ jobDetail.PBANC_CONT }}</h2>
                <h5>접수기간 : {{ jobDetail.RCPT_BGNG_DE }} ~ {{ jobDetail.RCPT_END_DE }} </h5>
            </div>
            <div class="buttons">
                <!-- 스크랩 버튼 -->
                ...
                <div class="homepagelink">
                    <a class="btn btn-primary" :href="jobDetail.URL" role="button" target="_blank">홈페이지 지원</a>
                </div>
            </div>
        </div>

        <div class="middle">
            <div>{{ jobDetail.PBANC_CONT }}</div>
            <br />
        ...
        <div class="bottom">
            ...
            <div>고용형태: {{ jobDetail.PBANC_FORM_DIV }}</div>
            <div>
                <h3>자격요건</h3>
            </div>
            <div>경력: {{ jobDetail.CAREER_DIV }}</div>
            <div>학력: {{ jobDetail.ACDMCR_DIV }}</div>
            <div>채용부문: {{ jobDetail.RECRUT_FIELD_NM }}</div>
            <div></div>
            <div>근무일: {{ jobDetail.HOLIDAY_NM }} <br />
                근무시간: {{ jobDetail.WORK_TM_NM }}, {{ jobDetail.WORK_TIME_NM }}
            </div>
            <div>급여: {{ jobDetail.SALARY_COND }}</div>
            <div>근무지역: {{ jobDetail.WORK_REGION_CONT }}</div>
        ...
        <div class="office">
            ...
            <div>회사위치: {{ jobDetail.WORK_REGION_CONT }}</div>
            <div>회사번호: {{ jobDetail.MNGR_PHON_NO }}</div>
            <div>회사이름: {{ jobDetail.ENTRPRS_NM }}</div>
...
const fetchJobData = async () => {
    try {
        const response = await axios.get(`http://localhost:8090/api/v1/jobaba/jobinfo/${jobId}`);
